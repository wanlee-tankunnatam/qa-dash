<template>
  <div class="flex flex-col h-full bg-white">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
      <h1 class="text-base font-semibold text-slate-800">AI Terminal</h1>
      <button
        ...
        @click="startMyDay"
      >
        <svg class="w-3.5 h-3.5" :class="isStreaming ? 'animate-spin' : ''" viewBox="0 0 16 16" fill="currentColor">
    ...
    <div ref="chatRef" class="flex-1 overflow-auto px-6 py-4 space-y-4">
      ...
      >
        ...
        <div v-if="msg.role === 'assistant'" class="flex items-start gap-2 max-w-[80%]">
          ...
          <div class="bg-slate-50 border border-slate-200 rounded-2xl rounded-tl-sm px-4 py-3">
            <pre class="text-sm text-slate-700 whitespace-pre-wrap break-words font-sans leading-relaxed">{{ msg.content }}<span v-if="isStreaming && i === messages.length - 1" class="inline-block w-1.5 h-4 bg-slate-400 animate-pulse ml-0.5 align-middle" /></pre>
          </div>
        </div>
...
async function sendPrompt() {
  ...
  scrollToBottom()
  try {
    await window.qaApi.ask(prompt || `อ่านและอธิบายไฟล์นี้: ${image}`, image || undefined)
  } catch (e) {
    streamError.value = (e as Error).message
    isStreaming.value = false
  }
